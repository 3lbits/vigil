...
func (m avvikModule) Register(deps modregistry.Dependencies, r *modregistry.Registrar) error {
	...
	submitOwnPolicy := modregistry.Policy{Resource: "avvik", Action: "submit_own"}
	managePolicy := modregistry.Policy{Resource: "admin", Action: "write"}
	avvikSubmitOwn := RequireAvvikSubmitOwn(deps.Queries, deps.Authz)
	...
	r.Guarded("POST /avvik/{id}/notes", submitOwnPolicy, h.AddNote, moduleGuard, avvikSubmitOwn)
	...
	r.Guarded("POST /avvik/{id}/attachments", submitOwnPolicy, h.AddAttachment, moduleGuard, avvikSubmitOwn)
